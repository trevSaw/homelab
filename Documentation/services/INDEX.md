# Services Index

| Service | Category | Migration Priority | Documentation Status | Link |
|---------|----------|-------------------|----------------------|------|
| authentic | Identity | P4‑P5 | present | ./authentic/authentic.md |
| beszel | Monitoring | P4‑P5 | present | ./beszel/beszel.md |
| beszel_agent | Monitoring | P4‑P5 | present | ./beszel_agent/beszel_agent.md |
| calibre-web | Productivity | P4‑P5 | present | ./calibre-web/calibre-web.md |
| code-server | Development | P4‑P5 | present | ./code-server/code-server.md |
| CosmoOS | Infrastructure | P4‑P5 | present | ./CosmoOS/CosmoOS.md |
| EchoOS | Utilities | P0 | present | ./EchoOS/EchoOS.md |
| echoos | Utilities | P0 | present | ./echoos/echoos.md |
| hermes | Messaging / Communication | P4‑P5 | present | ./hermes/hermes.md |
| homepage | Infrastructure / Reverse Proxy | P4‑P5 | present | ./homepage/homepage.md |
| homeassistant | Smart Home | High | present | ./homeassistant/service.md |
| honcho | Automation | P4‑P5 | present | ./honcho/honcho.md |
| Hotio | Media | P4‑P5 | present | ./Hotio/Hotio.md |
| jellyfin | Media | P4‑P5 | present | ./jellyfin/jellyfin.md |
| n8n | Automation | P4‑P5 | present | ./n8n/n8n.md |
| NZBget | Utilities | P4‑P5 | present | ./NZBget/NZBget.md |
| odysseus | AI | P1 | present | ./odysseus/odysseus.md |
| ollama | AI | P2 | present | ./ollama/ollama.md |
| portainer | Infrastructure / Management | P3 | present | ./portainer/portainer.md |
| traefik | Reverse Proxy / Networking | P2 | present | ./traefik/traefik.md |
| prowlarr | Media | P4‑P5 | present | ./prowlarr/prowlarr.md |
| radarr | Media | P4‑P5 | present | ./radarr/radarr.md |
| jellyseerr | Media | P4‑P5 | present | ./jellyseerr/jellyseerr.md |
| bazarr | Media | P4‑P5 | present | ./bazarr/bazarr.md |
| actual | Productivity | P4‑P5 | present | ./actual/actual.md |
| lazylibrarian | Media | P4‑P5 | present | ./lazylibrarian/lazylibrarian.md |
| crafty | Games | P4‑P5 | present | ./crafty/crafty.md |
| mariadb | Databases | P4‑P5 | present | ./mariadb/mariadb.md |
| uptime-kuma | Monitoring | P4‑P5 | present | ./uptime-kuma/uptime-kuma.md |
| byparr | Utilities | P4‑P5 | present | ./byparr/byparr.md |
| readarr | Media | P4‑P5 | present | ./readarr/readarr.md |
| kavita | Media | P4‑P5 | present | ./kavita/kavita.md |
| nextcloud | Productivity | P4‑P5 | present | ./nextcloud/nextcloud.md |
